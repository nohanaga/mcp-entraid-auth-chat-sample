        ...
        if hasattr(ctx, 'request_context') and ctx.request_context:
            ...
            if auth_header.startswith("Bearer "):
                ...
                if exp_claim and current_time < exp_claim:
                    ...
                    exp_time = datetime.fromtimestamp(exp_claim)
                    validation_results.append(f"✅ Expiration (exp): {exp_time} - VALID (not expired)")
                else:
                    validation_results.append("✗ Expiration (exp): Token EXPIRED")
                
                if nbf_claim and current_time >= nbf_claim:
                    from datetime import datetime
                    nbf_time = datetime.fromtimestamp(nbf_claim)
                    validation_results.append(f"✅ Not Before (nbf): {nbf_time} - VALID (token is active)")
                else:
...
@mcp.tool()
async def exchange_token_for_resource(ctx: Context, resource_uri: str) -> dict:
    """
    OBOフローでdownstream API用のアクセストークンを取得
    
    Frontend から受け取った委任アクセストークン (delegated access token) を、
    指定された downstream API 用のアクセストークンに交換します。
    Microsoft Entra ID の On-Behalf-Of フローと User-assigned Managed Identity を使用。
    ...
    Args:
        ctx: FastMCPコンテキスト (自動注入)
        resource_uri: Downstream API のリソース URI 
                     (例: https://kusto.kusto.windows.net, 
                          https://analysis.windows.net/powerbi/api)
    
    Returns:
        dict: OBO フローで取得したアクセストークンの情報
    """
    logger.info("=" * 80)
    logger.info(f"OBO Token Exchange requested for resource: {resource_uri}")

    logger.info(f"OBO Flow Enabled: {obo_config.use_obo_flow}")
    logger.info("=" * 80)
    ...
    try:
        ...
        # OBO フローで委任アクセストークンを downstream API 用のアクセストークンに交換
        ...
        obo_token = await token_exchanger.perform_obo_token_exchange(
            ...
            resource_uri=resource_uri  # Downstream API のリソース URI
        )
        ...
        logger.info(f"  Target Audience: {decoded_obo_token.get('aud')}")
        logger.info(f"  Scopes: {decoded_obo_token.get('scp', decoded_obo_token.get('roles'))}")
        
        return {
            "ok": True,
            "message": "OBO token exchange successful",
            "resource_uri": resource_uri,
            "token_info": {
                "audience": decoded_obo_token.get("aud"),
                "issuer": decoded_obo_token.get("iss"),
                "scopes": decoded_obo_token.get("scp", decoded_obo_token.get("roles")),
                "expires_at": decoded_obo_token.get("exp"),
                "subject": decoded_obo_token.get("sub"),
                "upn": decoded_obo_token.get("upn", decoded_obo_token.get("unique_name")),
            },
            "note": "This OBO token can be used to call the downstream API"
        }
...
async def get_azure_ai_search_token(ctx: Context) -> dict:
    """
    Azure AI Search 用の OBO トークンを取得 (DEMO用)
    
    Frontend から受け取った委任アクセストークン (delegated access token) を、
    On-Behalf-Of フローで Azure AI Search 用のアクセストークンに交換します。
    
    フロー:
    1. Frontend → Backend: 委任アクセストークン (aud: api://{BACKEND_API})
    2. Backend → Entra ID: OBO フローでトークン交換リクエスト
    3. Entra ID → Backend: OBO トークン (aud: https://search.azure.com)
    4. Backend → AI Search: OBO トークンで API 呼び出し
    ...
    Returns:
        dict: Azure AI Search 用の OBO トークン情報
    """
    logger.info("=" * 80)
    logger.info("Azure AI Search Token Request (DEMO)")
    logger.info("=" * 80)
    ...
    try:
        ...
        # OBO フローで Azure AI Search 用のアクセストークンを取得
        ...
        azure_search_resource = obo_config.target_audiences[0]
        logger.info(f"🔄 Exchanging delegated token for Azure AI Search OBO token: {azure_search_resource}")
        
        token_exchanger = TokenOboExchanger()
        obo_token = await token_exchanger.perform_obo_token_exchange(
            user_token=delegated_token,  # Frontend から受け取った委任アクセストークン
            resource_uri=azure_search_resource  # Azure AI Search のリソース URI
        )
        
        # コンソールに OBO トークン (生データ) を出力
        print("=" * 80)
        print("🔑 Azure AI Search OBO Token (Raw)")
        print("=" * 80)
        print(obo_token)
        print("=" * 80)
        ...
        logger.info("✅ Azure AI Search OBO token acquired successfully")
        logger.info(f"  Audience: {decoded_obo_token.get('aud')}")
        logger.info(f"  Scopes: {decoded_obo_token.get('scp', decoded_obo_token.get('roles'))}")
        
        return {
            "ok": True,
            "message": "Azure AI Search OBO token acquired successfully",
            "resource": azure_search_resource,
            ...
            "demo_note": "This OBO token can be used to call Azure AI Search APIs on behalf of the user"
        }
        ...
        logger.error(f"❌ Azure AI Search token exchange failed: {e}")
        return {
            "ok": False,
            "error": "token_exchange_failed",
            "message": str(e),
